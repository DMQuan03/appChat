...
export const Films = ({ data }) => {
    ...
    const users = sessionStorage.getItem("username")
    const params = useParams()

    const handleDelete = () => {
        try {
            axios.delete(`http://localhost:6875/v1/films/${data._id}/delete`)
            navigate("/addfilms")
            setTimeout(() => {
                navigate("/search")
            }, 1);
    ...
    return (
        <div className={cx("wrapper")}>
            <div className={cx("des-film")}>
                ...
                >
                    <img style={{ position: "absolute", left: 0, marginLeft: 4, marginTop: 5 }} src={data.avtUser} />
                    ...
                    {data.username === users ? <button style={{ position: "absolute", right: 0, marginRight: 15, marginTop: -20, borderRadius: "50%" }}
